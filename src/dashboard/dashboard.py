...
class Dashcard(View, metaclass=MediaDefiningClass):
    ...
    def chartdata(self, request, queryset=None):
        ...
        if self.chart_type == Dashcard.TYPE_TABLE:
            context = config.pop("data")
            context["label_name"] = self.label_name
            config["html"] = self.html_table(request, context=context)
    ...
    def html_table(self, request, context):
        if self.template_table:
            template_names = [self.template_table]
        else:
            template_names = [
                f"dashboard/{self.app_config.label}/{self.dash_name}_table.html",
                f"dashboard/{self.app_config.label}/dash_table.html",
                f"dashboard/dashcard_table.html",
            ]
